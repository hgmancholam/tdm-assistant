...
> This is the master email format rule. All skills that send email inherit this standard.

- **No icons, no emojis, no colored banners, no badges** — none, ever, in any email HTML
- **No inline color** — the only colors allowed are `#1a1a1a` (body text), `#555555` (signature subtitle), `#cccccc`/`#d8d8d8` (table/rule borders), `#ffffff` (background)
- **Health status exception (only):** the words `GREEN` / `YELLOW` / `RED` written as plain bold inline text — e.g., `<strong>Status: RED</strong>`. Never as a background color, badge, or colored box.
- **Explicitly prohibited CSS:** `background-color` (except `#ffffff`), `border-left: X solid [color]` decorative, `border-radius`, `box-shadow`, any color value not listed above
- **Subject line**: plain ASCII text only — e.g., `GTTH Project Status - Touch Point | June 18, 2026`
- **Structure via bold + ruled lines only** — section headers are `<strong>` with a `border-bottom: 1px solid #cccccc`
- **Tables**: 2px black bottom border on header row, 1px `#d8d8d8` on data rows — no background fills, no colored headers
- **Font**: Calibri 11pt — matches Outlook default, indistinguishable from a manually typed email
- **Layout**: `<table>`-based only (Outlook desktop uses Word renderer — no flexbox, grid, border-radius)
